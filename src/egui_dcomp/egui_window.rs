...
use super::egui_dcomp_renderer::EguiDcompRenderer;
use super::EguiView;

pub struct EguiWindow {
    pub window: Arc<Window>,
    pub renderer: EguiDcompRenderer,
...
impl EguiWindow {
    pub fn new(window: Arc<Window>, view: impl EguiView + 'static) -> anyhow::Result<Self> {
        let renderer = EguiDcompRenderer::new(&window)?;
    ...
    pub fn handle_resized(&mut self, size: PhysicalSize<u32>) {
        self.renderer.handle_resized(size.width, size.height);
    }
    ...
    pub fn handle_redraw(&mut self) -> anyhow::Result<()> {
        self.renderer
            .handle_redraw(&self.window, self.view.as_mut())
    }
}
